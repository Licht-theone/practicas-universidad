 ...
 * Compara eliminaOcurrenciasRapido() con eliminaOcurrenciasLento().
 * 
 * @author Estructuras de Datos
 * @version sep-2023
 */
public class MideTiempos2ComparaLentoRapido {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < tamanhoEjemplares.length; i++) {
			...
			for (int j = 0; j < NUM_REP_MEDIDA; j++) {
				// algoritmo lento
				EliminaOcurrencias.eliminaOcurrenciasLento(lista2, ELEM_ELIMINAR);
			}
			...
			System.out.printf("           Algoritmo lento:    %8.2fms %n", medidas2[i].ms());
			...
			if (!lista1.equals(lista2)) {
				System.out.println("Error: el algoritmo mas rapido no funciona correctamente");
				System.exit(-1);
			}
		}
