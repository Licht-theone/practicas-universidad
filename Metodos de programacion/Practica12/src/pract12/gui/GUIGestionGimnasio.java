...
import pract12.modelo.Gimnasio;
import pract12.modelo.Gimnasio.NoExisteActividad;
import pract12.modelo.Gimnasio.NoExisteSocio;
import pract12.modelo.Socio;
import pract12.modelo.Socio.SocioMaxActividades;
import pract12.modelo.Socio.SocioYaInscrito;
import pract12.modelo.SocioPlus;
...
public class GUIGestionGimnasio {
	...
	public static final int FICH_INFORME_ACTIVIDAD = 3;
	...
	public static void main(String[] args) {
		...
		Lectura lect;
		Gimnasio smartfit = null;
		// Crea el gimnasio
		try {
			smartfit = new Gimnasio();
		} catch (FileNotFoundException e) {
			mensaje("ERROR", "Fichero no encontrado");
			System.exit(-1);
		}
		...
		menu.insertaOpcion("Fichero informe actividad", FICH_INFORME_ACTIVIDAD);
		int opcion;
		...
		while (true) {
			...
			case INSCRIPCION_SOCIO:
				...
				break;
			case FICH_INFORME_ACTIVIDAD:
				lect = FundamentosFactory.getLectura("Fichero informe actividad");
				lect.creaEntrada("Nombre actividad", "Pilates");
				lect.esperaYCierra();
				nombreActividad = lect.leeString("Nombre actividad");
				try {
					smartfit.informeActividad(nombreActividad);
				} catch (IOException e) {
					mensaje("ERROR", "Error escribiendo el fichero");
					break;
				} catch (NoExisteActividad e) {
					mensaje("ERROR", "No existe ninguna actividad con el nombre indicado");
					break;
				}
				mensaje("Informe Actividad", "Generado el fichero " + nombreActividad + ".txt");
				break;
			default:
