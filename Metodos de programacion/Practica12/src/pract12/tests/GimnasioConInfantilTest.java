...
class GimnasioConInfantilTest {
	...
	private static final String[] DNIS_INFANTIL = {"30000000C", "30000001C"};
	...
	private static final int LIMITE_ACTIVIDADES_INANTIL = 2;
	private static final int LIMITE_ACTIVIDADES_PLUS = 10;

	private static final float PRECIO_FIJO_BASICO = 40;
	private static final float PRECIO_FIJO_INFANTIL = 20;
	...
	@Test
	void inscribeSocioErrorLimiteActividadesInfantilTest() {
		System.out.println("inscribeSocioErrorLimiteActividadesInfantilTest");

		// Inscribe socio basico en su limite de actividades
		for (int i = 0; i < LIMITE_ACTIVIDADES_INANTIL; i++) {
			FundamentosTest.interaccionGuiOK(INSCRIBE_EN_ACTIVIDAD,
					DNIS_INFANTIL[0], ACTIVIDADES[i]);
		}

		// comprueba que falla al tratar de inscribirle en una mas
		FundamentosTest.interaccionGUI(INSCRIBE_EN_ACTIVIDAD, "limite",
				DNIS_INFANTIL[0], ACTIVIDADES[LIMITE_ACTIVIDADES_INANTIL]);		
	}
	...
	@Test
	void datosInicialesSociosInfantilesTest() {
		System.out.println("datosInicialesSociosInfantilesTest");

		// comprueba que los socios basicos comienzan con pago 40 y 0 actividades
		for (String dni: DNIS_INFANTIL) {
			String respuesta =
					FundamentosTest.interaccionGuiOK(MUESTRA_DATOS_SOCIO, dni);
			compruebaDatosSocio(respuesta, PRECIO_FIJO_INFANTIL, 0, null);
		}
	}
	...
	@Test
	void pagoSocioInfantilTest() {
		System.out.println("pagoSocioInfantilTest");

		// comprueba que los socios infantiles comienzan con pago 20 y 0 actividades
		for (String dni: DNIS_INFANTIL) {
			String respuesta =
					FundamentosTest.interaccionGuiOK(MUESTRA_DATOS_SOCIO, dni);
			compruebaDatosSocio(respuesta, PRECIO_FIJO_INFANTIL, 0, null);
		...
		// inscribe los socios infantiles en actividades
		for (int i = 0; i < DNIS_INFANTIL.length; i++) {
			FundamentosTest.interaccionGuiOK(INSCRIBE_EN_ACTIVIDAD,
					DNIS_INFANTIL[i], ACTIVIDADES[i]);
		}
		for (int i = 0; i < DNIS_INFANTIL.length; i++) {
			FundamentosTest.interaccionGuiOK(INSCRIBE_EN_ACTIVIDAD,
					DNIS_INFANTIL[i], ACTIVIDADES[i + 1]);
		}

		// comprueba que los socios infantiles siguen pagando lo mismo
		for (String dni: DNIS_INFANTIL) {
			String respuesta =
					FundamentosTest.interaccionGuiOK(MUESTRA_DATOS_SOCIO, dni);
			compruebaDatosSocio(respuesta, PRECIO_FIJO_INFANTIL, 2, null);
		}
	}
